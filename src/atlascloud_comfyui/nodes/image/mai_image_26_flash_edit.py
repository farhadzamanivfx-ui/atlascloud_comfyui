...
class AtlasMAIImage26FlashEdit:
    ...
    def run(
        ...
        if not prompt:
            raise RuntimeError("prompt is required for MAI-Image-2.6-Flash Edit")

        image_list: List[str] = [v.strip() for v in (reference_images or "").splitlines() if v.strip()]
        if not image_list:
            raise RuntimeError("reference_images is required for MAI-Image-2.6-Flash Edit")
        if len(image_list) > 5:
        ...
        payload: Dict[str, Any] = {
            "model": "microsoft/mai-image-2.6-flash/edit",
            "prompt": prompt,
