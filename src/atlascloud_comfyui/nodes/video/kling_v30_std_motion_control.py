...
class AtlasKlingV30StdMotionControl:
    ...
    def run(
        ...
        if not (image or "").strip():
            raise RuntimeError("image is required for Kling V3.0 Std Motion-Control")
        if not (video or "").strip():
            raise RuntimeError("video is required for Kling V3.0 Std Motion-Control")
...
NODE_CLASS_MAPPINGS = {"AtlasCloud Kling V3.0 Std Motion-Control": AtlasKlingV30StdMotionControl}
NODE_DISPLAY_NAME_MAPPINGS = {"AtlasCloud Kling V3.0 Std Motion-Control": "AtlasCloud Kling V3.0 Std Motion-Control"}
